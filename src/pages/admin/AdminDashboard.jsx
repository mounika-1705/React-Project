...
//           <label>Ward</label>
//           <input placeholder="Enter ward" value={filters.ward} onChange={(e) =>
...
//           }
//           >
//             {["All", "Open", "Assigned","In Progress", "Completed", "Rejected",].map(
//               (s) => (
...
//             <select value={filters.wasteType} onChange={(e) =>
...
//                     <select value={selected.assignedOperator?._id || ""} onChange={(e) =>
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    setSummary({
      todayRequests: normalized.length,
      pendingRequests: normalized.filter(
        (r) => r.status !== "Completed" && r.status !== "Rejected"
      ).length,
      completedToday: normalized.filter(
        (r) => r.status === "Completed"
      ).length,
      slaBreaches: normalized.length,
    });
  }, []);
        ...
        ? {
            ...
            notes: [
              ...(r.notes || []),
              {
                ...
                text: `Assigned operator ${operator?.name}`,
              },
            ],
          }
        : r
  ...
  return (
    <div className="admin-manage">
      ...
      <select
        ...
        onChange={(e) => updateStatus(selected._id, e.target.value)}
      >
        {["Open", "Assigned", "In Progress", "Completed", "Rejected"].map(
          (s) => (
            <option key={s}>{s}</option>
          )
        )}
